...
func ConvertPDFText(path string) (BodyResult, MetaResult, error) {
	...
	go func() {
		...
		for _, line := range strings.Split(string(metaStr), "\n") {
			if parts := strings.SplitN(line, ":", 2); len(parts) > 1 {
				metaResult.meta[strings.ToLower(strings.TrimSpace(parts[0]))] = strings.TrimSpace(parts[1])
			}
		}
	...
	go func() {
		body, err := exec.Command("pdftotext", "-x", "0", "-y", "50", "-W", "595", "-H", "770", "-q", "-nopgbrk", "-layout", "-enc", "UTF-8", "-eol", "unix", path, "-").Output()
		if err != nil {
			bodyResult.err = err
		}
